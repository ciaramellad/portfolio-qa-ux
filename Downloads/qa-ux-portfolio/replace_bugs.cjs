...
const bugsEs = [
  {
    ...
    steps: [
      ...
      "Documentar qué elementos reciben foco y cuáles son omitidos"
    ],
    expected: "Todos los elementos interactivos (botones, enlaces, inputs) deben ser alcanzables mediante Tab en orden lógico. Ningún elemento interactivo debe ser omitido por la navegación de teclado (WCAG 2.1 SC 2.1.1).",
    ...
    impact: "Bloqueo total de navegación para usuarios que dependen del teclado (motriz reducida, usuarios de switch access). Incumple WCAG 2.1 nivel A — el nivel mínimo de conformidad.",
  ...
  },
  {
    ...
    severity: "High",
    wcag: "WCAG 2.1 SC 2.4.7 (Foco visible) — Nivel AA",
    ...
    steps: [
      ...
      "Documentar en qué elementos el foco no es visible"
    ],
    expected: "Cada elemento que recibe el foco debe mostrar un indicador visual claro (outline de al menos 2px, contraste mínimo 3:1 contra el fondo). El usuario debe saber en todo momento dónde está el foco (WCAG 2.1 SC 2.4.7).",
    ...
    impact: "El usuario pierde completamente la referencia de posición durante la navegación. Afecta especialmente a usuarios con baja visión que usan teclado. Incumple WCAG 2.1 AA.",
  ...
  },
  {
    ...
    severity: "High",
    wcag: "WCAG 2.1 SC 1.4.3 (Contraste mínimo) — Nivel AA",
    precondition: "Navegador: Chrome (última versión)\nURL: https://www.ryanair.com/es/es\nHerramienta: Axe DevTools o extensión Colour Contrast Analyser\nPantalla con configuración de color estándar (sin alto contraste activado)",
    steps: [
      "Abrir ryanair.com/es/es en Chrome",
      ...
      "Usar WebAIM Contrast Checker para confirmar ratios"
    ],
    expected: "Texto normal (< 18pt): ratio mínimo 4.5:1 contra el fondo. Texto grande (≥ 18pt o 14pt bold): ratio mínimo 3:1. Componentes UI (bordes de inputs, iconos): ratio mínimo 3:1 (WCAG 2.1 SC 1.4.3).",
    actual: "Axe DevTools detecta fallos de contraste en: texto de precio en gris claro sobre fondo blanco (ratio ~2.8:1), texto de etiquetas sobre el azul corporativo de Ryanair (#073590) en algunos botones secundarios, y placeholders de los campos del buscador. Ninguno alcanza el mínimo 4.5:1.",
    impact: "Dificulta o impide la lectura para usuarios con baja visión, daltonismo o en condiciones de luz intensa (móvil al sol). Incumple WCAG 2.1 AA.",
    evidence: null
  },
  {
    id: "BUG-004",
    ...
    severity: "High",
    wcag: "WCAG 2.1 SC 1.3.1 / 3.3.2 (Etiquetas de formulario) — Nivel A/AA",
    ...
    expected: "Cada campo del formulario debe tener una etiqueta accesible: <label> asociado por 'for/id', o atributo aria-label, o aria-labelledby. El lector de pantalla debe anunciar el propósito del campo al recibir el foco (WCAG 2.1 SC 1.3.1 y 3.3.2).",
    ...
    impact: "Usuarios de lectores de pantalla (ciegos, baja visión severa) no pueden identificar el propósito de los campos del buscador principal. Impide completar la tarea principal del sitio: reservar un vuelo. Incumple WCAG 2.1 nivel A.",
    evidence: null
  },
  {
    id: "BUG-005",
    ...
    severity: "Medium",
    wcag: "WCAG 2.1 SC 3.3.1 (Identificación de errores) — Nivel A",
    ...
    expected: "Al intentar buscar sin completar campos obligatorios, el sistema debe: 1. Mostrar un mensaje de error descriptivo junto a cada campo vacío. 2. Mover el foco al primer error o anunciarlo mediante aria-live. 3. El mensaje debe identificar el campo y cómo corregirlo (WCAG 2.1 SC 3.3.1 y 3.3.3).",
    ...
    impact: "Usuarios con discapacidad cognitiva o visual no saben por qué la búsqueda falla ni cómo resolverlo. Genera frustración y abandono del flujo principal. Incumple WCAG 2.1 SC 3.3.1 (nivel A).",
    evidence: null
  },
  {
    id: "BUG-006",
    ...
    severity: "Medium",
    wcag: "WCAG 2.1 SC 2.4.3 (Orden del foco) — Nivel A",
    ...
    expected: "El orden de tabulación debe seguir la secuencia lógica y visual: header → navegación principal → buscador → contenido → footer. Debe ser predecible y no confundir al usuario (WCAG 2.1 SC 2.4.3).",
    ...
    impact: "Confunde y desorenta a usuarios de teclado y lectores de pantalla. Hace impredicible la navegación y aumenta la carga cognitiva. Incumple WCAG 2.1 SC 2.4.3 (nivel A).",
    evidence: null
  },
  {
    id: "BUG-007",
    ...
    severity: "High",
    wcag: "WCAG 2.1 SC 4.1.2 (Nombre, función, valor) — Nivel A",
    ...
    expected: "Todos los elementos interactivos deben tener su rol correctamente definido (button, link, combobox, etc.) y un nombre accesible (aria-label o texto visible). Los lectores de pantalla deben anunciar: nombre + rol + estado del elemento (WCAG 2.1 SC 4.1.2).",
    ...
    impact: "Usuarios de lectores de pantalla no pueden identificar ni activar elementos interactivos clave. Incumple WCAG 2.1 SC 4.1.2 (nivel A) — uno de los criterios de más alto impacto en accesibilidad.",
    evidence: null
  },
  {
    id: "BUG-008",
    ...
    severity: "Low",
    wcag: "WCAG 2.1 SC 1.4.8 (Presentación visual) — Nivel AAA / Buena práctica",
    ...
    impact: "Reduce la velocidad de lectura hasta un 13-20% según estudios de legibilidad. Afecta a usuarios con dislexia y baja visión. Algunos lectores de pantalla pronuncian las mayúsculas hardcodeadas letra a letra en lugar de como palabras.",
    evidence: null
  },
  {
    id: "BUG-009",
    ...
    severity: "Medium",
    wcag: "WCAG 2.1 SC 1.1.1 (Contenido no textual) — Nivel A",
    ...
    expected: "Imágenes informativas (destinos, banners con información): deben tener alt descriptivo (ej: alt='Vista aérea de Roma, destino disponible'). Imágenes decorativas: deben tener alt='' para que el lector de pantalla las ignore (WCAG 2.1 SC 1.1.1).",
    ...
    impact: "Usuarios ciegos o con baja visión severa no reciben información sobre el contenido visual de las tarjetas de destinos, que es contenido informativo central del sitio. Incumple WCAG 2.1 SC 1.1.1 (nivel A).",
    evidence: null
  },
  {
    id: "BUG-010",
    ...
    severity: "High",
    wcag: "WCAG 2.1 SC 1.3.1 (Información y relaciones) — Nivel A",
    ...
    expected: "Cada página debe tener exactamente 1 elemento H1 que describa el contenido principal. La jerarquía debe ser continua y lógica: H1 → H2 → H3, sin saltos de nivel. Los headings deben representar la estructura real del contenido, no usarse como estilos visuales (WCAG 2.1 SC 1.3.1).",
    ...
    impact: "Los lectores de pantalla usan los headings como método de navegación rápida (tecla H en NVDA/JAWS). Una jerarquía rota impide la navegación eficiente y desorenta al usuario sobre la estructura del contenido. Incumple WCAG 2.1 SC 1.3.1 (nivel A).",
    evidence: null
  }
];

const bugsEn = bugsEs.map(b => ({...b})); // Use identical for now, except translate severities?
bugsEn.forEach(b => {
  // if High -> High, etc. Wait, severity strings are already High/Medium/Low
});

const file = path.join(__dirname, 'src', 'shared.tsx');
let content = fs.readFileSync(file, 'utf8');

// The easiest way to replace safely is via regex or string slice matching the exact block
function replaceBugs(str, langKey) {
  // Find `bugs: [` inside `ux_audit: { ... }` in the specific `lang` object
  // It's tricky with regex, let's just do a specific string replace.
  // We'll format the bugs manually to a string
  const stringified = JSON.stringify(langKey === 'es' ? bugsEs : bugsEn, null, 10).replace(/"(High|Medium|Low)"/g, '"$1"');
  // wait we need to fix the formatting inside the file.
  // Actually, I'll just use a regex that matches `bugs: [ ... ],` after `bugsTitle: "BUGS ENCONTRADOS"` or `bugsTitle: "BUGS FOUND"`
  ...
  if (p1 === -1) {
    console.error('NOT FOUND:', targetTitle);
    return str;
  }
  let p2 = str.indexOf('uxImprovementsTitle:', p1);
  if (p2 === -1) {
    console.error('NOT FOUND uxImprovementsTitle');
    return str;
  }
  
  // Format the bugs array without breaking the JSON too much
  // Make evidence undefined if null so it doesn't render
  const formattedBugs = (langKey === 'es' ? bugsEs : bugsEn).map(b => {
    let out = `        {
          ...
          impact: ${JSON.stringify(b.impact)}`;
...
console.log('Bugs replaced successfully!');
