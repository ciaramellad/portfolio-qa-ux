...
const BugCard: React.FC<BugCardProps> = ({ id, testCaseId, title, severity, type, description, steps, expected, actual, evidence, theme, lang, onViewEvidence }) => {
  ...
  const severityStyles = {
    High: {
      badge: isDark ? 'bg-red-500/15 text-red-400 border-red-500/20' : 'bg-red-50 text-red-600 border-red-200',
      dot: 'bg-red-500',
      button: isDark ? 'text-red-400 bg-red-500/10 hover:bg-red-500/20' : 'text-red-600 bg-red-50 hover:bg-red-100',
    },
    Medium: {
      badge: isDark ? 'bg-yellow-500/15 text-yellow-400 border-yellow-500/20' : 'bg-yellow-50 text-yellow-600 border-yellow-200',
      dot: 'bg-yellow-400',
      button: isDark ? 'text-yellow-400 bg-yellow-500/10 hover:bg-yellow-500/20' : 'text-yellow-600 bg-yellow-50 hover:bg-yellow-100',
    },
    Low: {
      badge: isDark ? 'bg-green-500/15 text-green-400 border-green-500/20' : 'bg-green-50 text-green-600 border-green-200',
      dot: 'bg-green-500',
      button: isDark ? 'text-green-400 bg-green-500/10 hover:bg-green-500/20' : 'text-green-600 bg-green-50 hover:bg-red-100',
    }
  }[severity];
  ...
  return (
    <div className={`rounded-2xl border flex flex-col h-full overflow-hidden transition-all duration-500
      ${isDark
        ...
        : 'bg-white border-black/5 shadow-[0_8px_32px_rgba(0,0,0,0.04)] hover:border-black/10'
      }`}>
      ...
      <div className={`flex items-center justify-between px-5 pt-5 pb-4`}>
        ...
        <span className={`flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full border ${severityStyles.badge}`}>
      ...
      {/* ── DIVIDER ── */}
      <div className={`mx-5 h-px ${isDark ? 'bg-white/6' : 'bg-black/6'}`} />
 
      {/* ── BODY ── */}
      <div className="flex flex-col flex-1 px-5 py-4 gap-4">
 
        {/* Title */}
        <h4 className={`text-sm md:text-base font-bold leading-snug tracking-tight
        ...
        <div className="flex-1">
          <span className={`block mb-1.5 text-[10px] uppercase tracking-widest font-bold
            ${isDark ? 'text-white/30' : 'text-black/35'}`}>
            {lang === 'es' ? 'Pasos' : 'Steps'}
          </span>
        ...
        <div className={`grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t
          ...
          <div>
            <span className={`block mb-1 text-[10px] uppercase tracking-widest font-bold
              ${isDark ? 'text-white/30' : 'text-black/35'}`}>
          ...
          <div>
            ...
            <p className="text-xs font-medium text-red-500 leading-relaxed">{actual}</p>
          </div>
        </div>
      </div>
      ...
      {(testCaseId || type || evidence) && (
        <div className={`flex flex-col sm:flex-row sm:items-center gap-3 px-5 py-4 border-t
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            {type && (
              <span className={`text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-md
                ${isDark ? 'bg-white/6 text-white/40' : 'bg-black/5 text-black/50'}`}>
                {type}
              </span>
            )}
          </div>
      ...
      <main className="relative z-10">
            ...
            <div className="relative w-full flex flex-col md:flex-row items-center justify-center min-h-[420px] gap-8 md:gap-12">
              ...
              <div className="flex flex-col gap-6 md:hidden w-full">
                {features.slice(2).map((feature, i) => (
                  <div key={i} className="text-center">
                    <div className={`w-[1px] h-6 mx-auto mb-2 ${isDark ? 'bg-white/20' : 'bg-black'}`}></div>
                    <p className="text-lg font-bold">{feature}</p>
                  </div>
                ))}
              </div>
        ...
        {statsTable && (
          <ProjectSection id="stats-table" theme={theme}>
            <span className={`section-subheading ${isDark ? 'opacity-50' : 'opacity-60'}`}>{getSectionNumber()} / {statsTable.title}</span>
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="text-xs md:text-sm">
                    {statsTable.rows.map((row, i) => (
                      <tr key={i} className={`border-b last:border-0 transition-colors ${isDark ? 'border-white/5 hover:bg-white/[0.02]' : 'border-black hover:bg-black/[0.01]'}`}>
                        {row.map((cell, j) => (
        ...
        {testingTypes && testingTypes.length > 0 && (
          <ProjectSection id="testing-types" theme={theme}>
            <div className={`p-8 md:p-12 rounded-[24px] backdrop-blur-md ${isDark ? "bg-white/90 text-[#121212]" : "bg-[#121212]/90 text-white"}`}>
              <span className={`section-subheading ${isDark ? 'opacity-50' : 'opacity-60'}`}>{getSectionNumber()} / {testingStrategyTitle || (lang === 'es' ? 'Tipos de Testing' : 'Testing Types')}</span>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
                {testingTypes.map((type, i) => (
                  <div key={i} className="flex flex-col gap-3">
                    <div className="flex items-center gap-3">
                      <type.icon className={`w-5 h-5 shrink-0 ${isDark ? 'text-black' : 'text-white'}`} />
                      <span className={`text-xs font-bold uppercase tracking-[0.2em] leading-tight ${isDark ? 'text-black' : 'text-white'}`}>{type.name}</span>
                    </div>
                    {type.description && (
                      <p className={`text-xs font-light leading-relaxed ${isDark ? 'text-black/70' : 'text-white/70'}`}>
                        {type.description}
                      </p>
                    )}
                  </div>
        ...
        {testScenarios && (
          <ProjectSection id="test-scenarios" theme={theme}>
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="text-xs md:text-sm">
                    {testScenarios.table.map((row, i) => (
                      <tr key={i} className={`border-b last:border-0 transition-colors ${isDark ? 'border-white/5 hover:bg-white/[0.02]' : 'border-black hover:bg-black/[0.01]'}`}>
                        <td className="py-3 px-4 md:px-6 font-mono text-xs">
                          {(() => {
                            let colorClass = '';
                        ...
                        {row.feature && <td className="py-3 px-4 md:px-6 font-bold tracking-tight text-xs md:text-sm">{row.feature}</td>}
                        <td className="py-3 px-4 md:px-6 font-light opacity-70 text-xs md:text-sm">{row.scenario}</td>
                        {row.objective && <td className="py-3 px-4 md:px-6 font-medium text-xs md:text-sm">{row.objective}</td>}
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {testScenarios.footer && (
              <p className={`mt-6 text-xs font-light opacity-40`}>{testScenarios.footer}</p>
            )}
          </ProjectSection>
        )}
        ...
        {testScenarios2 && (
          <ProjectSection id="test-scenarios-2" theme={theme}>
            ...
            {testScenarios2.footer && (
              <p className={`mt-6 text-xs font-light opacity-40`}>{testScenarios2.footer}</p>
            )}
          </ProjectSection>
        )}
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="text-xs md:text-sm">
                    {testScenariosTable.table.map((row, i) => (
                      <tr key={i} className={`border-b last:border-0 transition-colors ${isDark ? 'border-white/5 hover:bg-white/[0.02]' : 'border-black hover:bg-black/[0.01]'}`}>
                        <td className="py-3 px-4 md:px-6 font-mono text-xs opacity-50">{row.id}</td>
                        <td className="py-3 px-4 md:px-6 font-bold tracking-tight text-xs md:text-sm">{row.feature}</td>
                        <td className="py-3 px-4 md:px-6 font-light opacity-70 text-xs md:text-sm">{row.scenario}</td>
                        <td className="py-3 px-4 md:px-6 font-medium text-xs md:text-sm">{row.objective}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="text-xs md:text-sm">
                    {testCasesSummary.table.map((row, i) => (
                      <tr key={i} className={`border-b last:border-0 transition-colors ${isDark ? 'border-white/5 hover:bg-white/[0.02]' : 'border-black hover:bg-black/[0.01]'}`}>
                        <td className="py-3 px-4 md:px-6 font-mono text-xs opacity-50">{row.scenario}</td>
                        <td className="py-3 px-4 md:px-6 font-mono text-xs opacity-50">{row.testCase}</td>
                        <td className="py-3 px-4 md:px-6 font-bold tracking-tight text-xs md:text-sm">{row.functionality}</td>
                        ...
                        <td className="py-3 px-4 md:px-6 font-mono text-xs text-red-500">{row.bug}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                              ...
                              {row.details.evidence && (
                                <div>
                                  <button 
                                    ...
                                    className="flex items-center gap-2 text-xs font-bold text-orange-500 hover:text-orange-600 transition-colors group"
                                  >
              ...
              {validations.items.map((item, i) => (
                <div key={i} className="space-y-4">
                  <div className="space-y-1.5">
                    ...
                    {item.description && (
                      <p className={`text-xs font-light opacity-60`}>{item.description}</p>
                    )}
                  </div>
                  <div className={`relative group rounded-2xl overflow-hidden border ${isDark ? 'bg-[#181818] border-white/10' : 'bg-black/5 border-black/10'}`}>
                    <div className={`flex items-center justify-between px-5 py-2.5 border-b ${isDark ? 'border-white/5 bg-white/5' : 'border-black/5 bg-black/5'}`}>
                      <div className="flex items-center gap-2">
        ...
        <ProjectSection id="contact" theme={theme} className="items-center text-center">
          ...
          <div className={`mt-20 text-xs uppercase tracking-[0.5em] font-medium ${isDark ? 'opacity-30' : 'opacity-50'}`}>
            {t.footer}
          </div>
        </ProjectSection>
