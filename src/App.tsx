    ...
    <motion.div 
      ...
      className={`group border p-6 md:p-8 transition-all duration-300 cursor-pointer h-full flex flex-col hover:shadow-2xl rounded-2xl backdrop-blur-sm
        ${isDark 
          ...
          : 'border-transparent text-white bg-black hover:bg-[#121212]'
        }`}
    >
      <div className="flex justify-between items-start mb-6">
        <CheckCircle2 className={`w-5 h-5 transition-opacity ${isDark ? 'opacity-40 group-hover:opacity-100' : 'opacity-100'}`} />
      </div>

      <h3 className="mb-3 tracking-tight">{title}</h3>

      <p className={`text-[16px] opacity-70 leading-relaxed transition-opacity group-hover:opacity-100`}>
        {description}
      </p>

      {link && (
        <div className="mt-auto pt-6 flex items-center gap-2 text-[14px] font-bold tracking-widest uppercase opacity-100 transition-opacity">
    ...
    <div className="w-full rounded-xl overflow-hidden border border-white/10">
      ...
      <AnimatePresence mode="wait" custom={direction}>
        ...
        >
          ...
          <div className={`w-full md:w-2/5 p-6 md:p-8 flex flex-col justify-center ${!isDark ? 'bg-black text-white' : ''}`}>
 
            <span className={`text-[14px] tracking-[0.2em] uppercase font-medium mb-4 block ${isDark ? 'opacity-70' : 'opacity-100'}`}>
            ...
            <p className={`leading-relaxed mb-8 ${isDark ? 'opacity-70' : 'opacity-100'}`}>
              {project.desc}
            </p>
        ...
        <Route path="/" element={
          <main className="relative z-10 w-full">
            ...
            <Section id="hero" theme={theme} className="items-center text-center">
              ...
              >
                <span className={`text-[14px] uppercase tracking-[0.3em] font-medium ${theme === 'dark' ? 'opacity-40' : 'opacity-80'}`}>{t.common.scroll}</span>
            ...
            <Section id="about" theme={theme}>
              <div className="w-full">
                ...
                <span className={`section-subheading ${theme === 'dark' ? 'opacity-50' : 'opacity-80'}`}>
                  01 / {t.about}
                </span>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-24">
                  <h2 className="font-bold tracking-tight m-0">
                    {t.aboutTitle}
                  </h2>
            ...
            <Section id="projects-qa" theme={theme}>
              <span className={`section-subheading ${theme === 'dark' ? 'opacity-50' : 'opacity-80'}`}>02 / {t.projectsQA}</span>
            ...
            <Section id="projects-ux-ui" theme={theme}>
              <span className={`section-subheading ${theme === 'dark' ? 'opacity-50' : 'opacity-80'}`}>03 / {t.visualProjectsTitle}</span>
              <AppleVisualSlider projects={t.visualProjects} theme={theme} t={t} />
            </Section>

            {/* SECTION 5 — SKILLS */}
            <Section id="skills" theme={theme}>
              <span className={`section-subheading ${theme === 'dark' ? 'opacity-50' : 'opacity-80'}`}>04 / {t.skills}</span>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-y-16 gap-x-12">
                {[
                  ...
                  <div key={i} className="group">
                    ...
                    <span className="text-[18px] tracking-wide">{skill}</span>
                  </div>
                ))}
              </div>
            </Section>
            ...
            <Section id="contact" theme={theme} className="items-center text-center">
              ...
              <div className={`mt-8 text-[14px] uppercase tracking-[0.5em] font-medium ${theme === 'dark' ? 'opacity-30' : 'opacity-70'}`}>
                {t.footer}
              </div>
            </Section>
